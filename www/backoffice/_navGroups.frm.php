...
@define('PREVIUS_PAGE', '_navGroups.lst');
@define('NEXT_PAGE',    '_navGroups.sbm');

$id = ($_GET["ref"]) ? f_decode(filter_var(trim($_GET["ref"]), FILTER_SANITIZE_SPECIAL_CHARS)) : null;

$action = 'ins';
$newUUID = Uuid::uuid4();
$navGroupUUID = $newUUID;
$navGroupStatus = 1;
$navGroupSpace = 0; // 0 = não, 1 = sim

if($id){
    $action = 'upd';
    $model = (new SysNavGroups())->findById($id);
    foreach ($model->Data() as $field => $value) {
        ${$field} = ($field !== "") ? trim($value) : null;
    }
    $navGroupUUID = (empty($navGroupUUID)) ? $newUUID : $navGroupUUID;
}
            ...
            <div class="col-md-12">
              <div class="grid simple">
                ...
                <div class="grid-body no-border">
                  ...
                  <input type="hidden" id="navGroupUUID" name="navGroupUUID" value="<?=$navGroupUUID?>">
                  <?php 
                  if($action="upd"){ 
                    echo ($navGroupID) ? "<input type='hidden' id='navGroupID' name='navGroupID' value='{$navGroupID}'>".chr(10) :null;
                  } // end if
                  ?>
                    <!-- row -->  
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            ...
                            <div class="controls">
                                <input type="text" id="navGroupName" name="navGroupName"  class="form-control" style="font-weight:normal"  value="<?=$navGroupName?>">
                                <label id="navGroupName" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-1 col-sm-1 col-xs-1">
                            ...
                            <span class="help"></span>
                            <div class="controls">
                                <input type="number" id="navGroupSeq" name="navGroupSeq"  class="form-control" style="font-weight:normal"  value="<?=$navGroupSeq?>">
                                <label id="navGroupSeq" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                            ...
                            <div class="controls">
                                ...
                                if($aSN["pt"]){
                                    foreach ($aSN["pt"] as $key => $value) {
                                        $checked = ($navGroupSpace == $key) ? 'checked' : '';
                                        echo "<div class='radio radio-success'>";
                                        echo    "<input type='radio' name='navGroupSpace' id='navGroupSpace$key' value='$key' $checked>";
                                        echo    "<label for='navGroupSpace$key'>$value</label>";
                                        echo "</div>";
                                    }
                                }
                                ?>
                                <label id="navGroupSpace" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                  ...
                  <div class="col-md-2 col-sm-2 col-xs-2">
                    <div class="form-group">
                      ...
                      <span class="help"></span>
                      <div class="controls">
                        <select class="form-control" name="navGroupStatus" id="navGroupStatus">
                          <?php 
                          if($aNavGroupsStatus){
                            foreach ($aNavGroupsStatus as $key => $value) {
                              $selected = ($key == $navGroupStatus) ? 'selected' : '';
                  ...
                  <div class="col-md-8 col-sm-8 col-xs-8">
                    <div class="form-group">
                      <div class="controls">
                        ...
                        <?=(isset($navGroupID)) ? "<button class='btn btn-danger' name='btnDelete' style='width:100px;margin-right:20px'>Excluir</button>":null; ?>
    ...
    <script>
      $(document).ready(function() {
        ...
        function formValidate(){
          ...
          if ($("input[name=navGroupName]").val() == "")  {
            err.push(['navGroupName', 'Informe o nome do Grupo !']);
          }
          if ($("input[name=navGroupSeq]").val() == "")  {
            err.push(['navGroupSeq', 'Informe a Sequencia de Exibição do Grupo !']);
          }
