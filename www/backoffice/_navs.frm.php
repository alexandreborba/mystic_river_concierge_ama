...
@define('PAGE_NAME',    'Item do Menu de Navegação');
@define('PREVIUS_PAGE', '_navs.lst');
@define('NEXT_PAGE',    '_navs.sbm');

include_once "__lstNavGroups.inc.php"; // lista de grupos de navegação

$id = ($_GET["ref"]) ? f_decode(filter_var(trim($_GET["ref"]), FILTER_SANITIZE_SPECIAL_CHARS)) : null;
...
$navStatus = 1;
$navSeq = 1;
$navSpace = 0; // 0 = não, 1 = sim

if($id){
    $action = 'upd';
    $model = (new SysNavs())->findById($id);
    foreach ($model->Data() as $field => $value) {
        ${$field} = ($field !== "") ? trim($value) : null;
    }
    $navUUID = (empty($navUUID)) ? $newUUID : $navUUID;
    include_once "__lstNavSubGroupsByGroup.inc.php"; // lista de grupos de navegação
}
  ...
  <body class="">
            ...
            <div class="col-md-12">
              <div class="grid simple">
                <div class="grid-title no-border" style="padding:10px 0px 10px 20px">
                  <h3><span class="semi-bold" style="color:#0146a0"><?=PAGE_NAME?></span></h3>
                  <hr>
                </div>
                ...
                <div class="grid-body no-border">
                  ...
                  <input type="hidden" id="navUUID" name="navUUID" value="<?=$navUUID?>">
                  <?php 
                  if($action="upd"){ 
                    echo ($navID) ? "<input type='hidden' id='navID' name='navID' value='{$navID}'>".chr(10) :null;
                  } // end if
                  ?>
                    <!-- row -->  
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            <div class="form-group">                        
                                ...
                                <div class="controls">
                                     <select class="form-control" name="navGroupUUID" id="navGroupUUID">
                                        <?php 
                                        if($lstNavGroups){
                                            foreach ($lstNavGroups as $key => $value) {
                                            $selected = ($key == $navGroupUUID) ? 'selected' : '';
                                            echo "<option value='$key' $selected >$value</option>";
                                            }
                                        } 
                                        ?>
                                    </select>
                                    <label id="navGroupUUID" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            <div class="form-group">                        
                                ...
                                <div class="controls">
                                     <select class="form-control" name="navSubGroupUUID" id="navSubGroupUUID">
                                        <?php 
                                        if($lstNavSubGroups){
                                            foreach ($lstNavSubGroups as $key => $value) {
                                            $selected = ($key == $navSubGroupUUID) ? 'selected' : '';
                                            echo "<option value='$key' $selected >$value</option>";
                                            }
                                        } else{
                                    ...
                                    <label id="navSubGroupUUID" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            ...
                            <div class="controls">
                                <input type="text" id="navName" name="navName"  class="form-control" style="font-weight:normal"  value="<?=$navName?>">
                                <label id="navName" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            <div class="form-group">
                            <label class="form-label">Link do Item</label>
                            <span class="help"></span>
                            <div class="controls">
                                <input type="text" id="navUrl" name="navUrl"  class="form-control" style="font-weight:normal"  value="<?=$navUrl?>">
                                <label id="navUrl" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-1 col-sm-1 col-xs-1">
                            ...
                            <span class="help"></span>
                            <div class="controls">
                                <input type="number" id="navSeq" name="navSeq"  class="form-control" style="font-weight:normal"  value="<?=$navSeq?>">
                                <label id="navSeq" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-2 col-sm-2 col-xs-2">
                            <div class="form-group">
                            <label class="form-label">Espaço Após</label>
                            <span class="help"></span>
                            <div class="controls">
                                <?php 
                                // sim ou não radio button
                                if($aSN["pt"]){
                                    foreach ($aSN["pt"] as $key => $value) {
                                        $checked = ($navSpace == $key) ? 'checked' : '';
                                        echo "<div class='radio radio-success'>";
                                        echo    "<input type='radio' name='navSpace' id='navSpace$key' value='$key' $checked>";
                                        echo    "<label for='navSpace$key'>$value</label>";
                                        echo "</div>";
                                    }
                                }
                                ?>
                                <label id="navSpace" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                  ...
                  <div class="col-md-2 col-sm-2 col-xs-2">
                    <div class="form-group">
                      ...
                      <span class="help"></span>
                      <div class="controls">
                        <select class="form-control" name="navStatus" id="navStatus">
                          <?php 
                          if($aNavsStatus){
                            foreach ($aNavsStatus as $key => $value) {
                              $selected = ($key == $navStatus) ? 'selected' : '';
                              echo "<option value='$key' $selected >$value</option>";
                            }
                          } 
                          ?>
                  ...
                  <div class="col-md-8 col-sm-8 col-xs-8">
                    <div class="form-group">
                      <div class="controls">
                        ...
                        <?=(isset($navID)) ? "<button class='btn btn-danger' name='btnDelete' style='width:100px;margin-right:20px'>Excluir</button>":null; ?>
    ...
    <script>
      $(document).ready(function() {
        ...
        $("select[name='navGroupUUID']").change(function(e){
            e.preventDefault();
            let navGroupUUID = $(this).val();
            
            // $("select[name='bUnitUUID']").html('<option value=""> -- antes deve selecionar um Prédio -- </option>');
            $("select[name='navSubGroupUUID']").html('<option value=""> aguarde, a carregar ... </option>');
            
            setTimeout(() => {  
                // get options from ajax
                $.ajax({
                url: '__ajax.getNavSubGroups.php',
                type: 'POST',
                data: {navGroupUUID: navGroupUUID},
                success: function(data){
                    console.log(data);
                    $("select[name='navSubGroupUUID']").html(data);
                },
                error: function(xhr, status, error){
                    console.error("Error fetching navSubGroupUUID: " + error);
                }
                });
             }, 500); // delay de 1 segundo para exibir mensagem de carregamento
        });
        
        $("button[name='btnDelete']").click(function(e){
          e.preventDefault();
          if(confirm('Deseja realmente excluir este Grupo de Navegação ?')){
        ...
        function formValidate(){
          ...
          if ($("select[name=navGroupUUID]").val() == "0")  {
            err.push(['navGroupUUID', 'Selecione o Grupo do Item !']);
          }

          if ($("select[name=navSubGroupUUID]").val() == "0")  {
            err.push(['navSubGroupUUID', 'Selecione o SubGrupo do Item !']);
          }

          if ($("input[name=navName]").val() == "")  {
            err.push(['navName', 'Informe o nome do Item !']);
          }

          if ($("input[name=navUrl]").val() == "")  {
            err.push(['navUrl', 'Informe o Link do Item !']);
          }

          if ($("input[name=navSeq]").val() == "")  {
            err.push(['navName', 'Informe a Sequencia de Exibição do Item !']);
          }
