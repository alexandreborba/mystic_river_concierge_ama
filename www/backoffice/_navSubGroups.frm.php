...
@define('PAGE_NAME',    'SubGrupos do Menu de Navegação');
@define('PREVIUS_PAGE', '_navSubGroups.lst');
@define('NEXT_PAGE',    '_navSubGroups.sbm');
...
$action = 'ins';
$newUUID = Uuid::uuid4();
$navSubGroupUUID = $newUUID;
$navSubGroupStatus = 1;
$navSubGroupSpace = 0; // 0 = não, 1 = sim

if($id){
    $action = 'upd';
    $model = (new SysNavSubGroups())->findById($id);
    foreach ($model->Data() as $field => $value) {
        ${$field} = ($field !== "") ? trim($value) : null;
    }
    $navSubGroupUUID = (empty($navSubGroupUUID)) ? $newUUID : $navSubGroupUUID;
}
            ...
            <div class="col-md-12">
              <div class="grid simple">
                ...
                <div class="grid-body no-border">
                  ...
                  <input type="hidden" id="navSubGroupUUID" name="navSubGroupUUID" value="<?=$navSubGroupUUID?>">
                  <?php 
                  if($action="upd"){ 
                    echo ($navSubGroupID) ? "<input type='hidden' id='navSubGroupID' name='navSubGroupID' value='{$navSubGroupID}'>".chr(10) :null;
                  } // end if
                  ?>
                    <!-- row -->  
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-3">
                            <div class="form-group">
                            <label class="form-label">Nome SubGrupo</label>
                            <span class="help"></span>
                            <div class="controls">
                                <input type="text" id="navSubGroupName" name="navSubGroupName"  class="form-control" style="font-weight:normal"  value="<?=$navSubGroupName?>">
                                <label id="navSubGroupName" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-1 col-sm-1 col-xs-1">
                            ...
                            <span class="help"></span>
                            <div class="controls">
                                <input type="number" id="navSubGroupSeq" name="navSubGroupSeq"  class="form-control" style="font-weight:normal"  value="<?=$navSubGroupSeq?>">
                                <label id="navSubGroupSeq" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                            ...
                            <div class="controls">
                                ...
                                if($aSN["pt"]){
                                    foreach ($aSN["pt"] as $key => $value) {
                                        $checked = ($navSubGroupSpace == $key) ? 'checked' : '';
                                        echo "<div class='radio radio-success'>";
                                        echo    "<input type='radio' name='navSubGroupSpace' id='navSubGroupSpace$key' value='$key' $checked>";
                                        echo    "<label for='navSubGroupSpace$key'>$value</label>";
                                        echo "</div>";
                                    }
                                }
                                ?>
                                <label id="navSubGroupSpace" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                            </div>
                        </div>
                  ...
                  <div class="col-md-2 col-sm-2 col-xs-2">
                    <div class="form-group">
                      ...
                      <span class="help"></span>
                      <div class="controls">
                        <select class="form-control" name="navSubGroupStatus" id="navSubGroupStatus">
                          <?php 
                          if($aNavSubGroupsStatus){
                            foreach ($aNavSubGroupsStatus as $key => $value) {
                              $selected = ($key == $navSubGroupStatus) ? 'selected' : '';
                              echo "<option value='$key' $selected >$value</option>";
                            }
                          } 
                          ?>
                  ...
                  <div class="col-md-8 col-sm-8 col-xs-8">
                    <div class="form-group">
                      <div class="controls">
                        ...
                        <?=(isset($navSubGroupID)) ? "<button class='btn btn-danger' name='btnDelete' style='width:100px;margin-right:20px'>Excluir</button>":null; ?>
    ...
    <script>
      $(document).ready(function() {
        ...
        function formValidate(){
          ...
          if ($("input[name=navSubGroupName]").val() == "")  {
            err.push(['navSubGroupName', 'Informe o nome do SubGrupo !']);
          }

          if ($("input[name=navSubGroupSeq]").val() == "")  {
            err.push(['navSubGroupSeq', 'Informe a Sequencia de Exibição do Subgrupo !']);
          }
