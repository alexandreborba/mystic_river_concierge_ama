...
@define('PAGE_NAME', 'DAILY MENUS');
@define('PREVIUS_PAGE', null);
@define('NEXT_PAGE', '_dailyMenus.frm');

@define('MODEL', 'DailyMenus');
@define('statusField', 'dmStatus');
@define('fieldQuery', 'dmUUID');
$sCode = shipCode;
?>
                  ...
                  <table class="table table-striped" id="datalist">
                      <thead>
                          <tr>
                              ...
                              <th style="text-align:left;width:15%"><?=$dict->translate('Type',$langCode);?></th>
                              <!-- <th style="text-align:left;width:auto">FILE</th> -->
                              <th style="text-align:center;width:7%"><?=$dict->translate('Size',$langCode);?></th>
                              <th style="text-align:center;width:10%"><?=$dict->translate('Dimensions',$langCode);?></th>
                              <th style="text-align:center;width:12%"><?=$dict->translate('Status',$langCode);?></th>
                        ...
                        <?php 
                            ...
                            $dailyMenus = (new DailyMenus())->find("shipCode=:ship AND deleted_at IS NULL","ship={$sCode}")->fetch(true);
                            if($dailyMenus){
                              
                        foreach($dailyMenus as $dailyMenu){
                    ?>
                    <tr class="odd gradeX">
                        <td style="text-align:center;vertical-align: middle;"><?=$dailyMenu->dmID?></td>
                        <!-- 
                        <td style="text-align:center;vertical-align: middle;">
                          <a href="<?=NEXT_PAGE?>?ref=<?=f_encode($dailyMenu->dmID)?>" ><?=$dailyMenu->dmUUID?></a>
                        </td>
                        -->
                        <!-- <td style="text-align:left;vertical-align: middle;font-weight:normal"><?=$dailyMenu->dmDate.' - '.f_dayOfWeek($dailyMenu->dmDate);?></td> -->
                        <td style="text-align:left;vertical-align: middle;font-weight:bold"><?=$dailyMenu->dmName?></td>
                        <td style="text-align:left;vertical-align: middle;font-weight:normal"><?=$lstMenuTypes[$dailyMenu->dmTypeUUID]?></td>
                        <!-- <td style="text-align:left;vertical-align: middle;font-weight:bold"><?=$dailyMenu->dmFileOriginal?></td> -->
                        <td style="text-align:center;vertical-align: middle;font-weight:normal"><?=f_filesSizeFormatShort($dailyMenu->dmFileSize)?></td>
                        <td style="text-align:center;vertical-align: middle;font-weight:normal"><?=$dailyMenu->dmFileDimensions.' px'?></td>
                        
                        <td style="text-align:center;vertical-align:middle">
                          <a href="#" name="changeThisStatus" title='<?=$dict->translate('Click here to Change Status!',$langCode)?>'data="<?=$dailyMenu->dmUUID?>,<?=statusField?>"  style='border-radius:10px;padding:8%' class="label <?= $aDmStatusLabel[$dailyMenu->dmStatus] ?>" >
                            <?= $aDmStatus[$dailyMenu->dmStatus] ?>
                          </a>
                        </td>
                        
                        <td class="center" style="text-align:center;vertical-align: middle;">
                            <a href="<?=NEXT_PAGE?>?ref=<?=f_encode($dailyMenu->dmID)?>" class="btn btn-success" style="width:100px;">Change</a>
                        </td>
                      </tr>
