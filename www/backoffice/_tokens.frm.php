...
@define('PAGE_NAME', 'TOKENS');
@define('NEXT_PAGE', '_tokens.sbm');
@define('PREVIUS_PAGE', '_tokens.lst');

$action = 'ins';
$newUUID = Uuid::uuid4();

$tokenUUID = $newUUID;
$tokenDateTime = f_datetime();
$tokenJWT = f_jwt_token_generate(shipCode.$tokenDateTime);
$tokenStatus = 1;

$tokenStartDateTime = f_datetime();
$tokenEndDateTime = f_addToDate($tokenStartDateTime, 2, 'y');

$id = ($_GET['ref']) ? f_decode(filter_var(trim($_GET['ref']), FILTER_SANITIZE_STRING)) : null;

if ($id) {
    $action = 'upd';
    $token = (new SysTokens())->findById($id);
    foreach ($token->Data() as $field => $value) {
        ${$field} = ($field !== '') ? trim($value) : null;
    }
    $tokenUUID = (empty($tokenUUID)) ? $newUUID : $tokenUUID;
    $tokenDateTime = (empty($tokenDateTime)) ? f_datetime() : $tokenDateTime;
    $tokenJWT = (empty($tokenJWT)) ? f_jwt_token_generate(shipCode) : $tokenJWT;
}
            ...
            <div class="col-md-12">
              <div class="grid simple">
                ...
                <form name="form" action="<?php echo NEXT_PAGE; ?>" method="post" enctype="multipart/form-data">
                  ...
                  <input type="hidden" id="tokenDateTime" name="tokenDateTime" value="<?=$tokenDateTime; ?>">
                  <input type="hidden" id="tokenJWT" name="tokenJWT" value="<?=$tokenJWT; ?>">
                  <?php
                  if ($action = 'upd') {
                      echo ($tokenID)   ? chr(10).chr(9).chr(9).chr(9).chr(9)."<input type='hidden' id='tokenID' name='tokenID' value='{$tokenID}'>".chr(10) : null;
                  }// end if
?>
                  <!-- row -->  
                  <div class="row">
                    ...
                    <div class="col-md-4 col-sm-4 col-xs-4">
                      <div class="form-group">                        
                        ...
                        <div class="controls">
                        <p style='font-family:roboto;color:#dadada;font-weight:bold'><?=$tokenUUID; ?></p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="col-md-4 col-sm-4 col-xs-4">
                      <div class="form-group">                        
                        ...
                        <div class="controls">
                        <p style='font-family:roboto;color:#005bc4;font-weight:bold'><?=$tokenDateTime; ?></p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-6">
                      <div class="form-group">
                        <label class="form-label">Token Name - Use / Need / Application or Reference</label>
                        <span class="help">Example: PowerBI Access</span>
                        <div class="controls">
                          <input type="text" id="tokenName" name="tokenName"  class="form-control" style="font-weight:bold"  value="<?=$tokenName; ?>">
                          <label id="tokenName" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-6">
                      <div class="form-group">
                        <label class="form-label">Token JWT String</label>
                        <span class="help"></span>
                        <div class="controls">
                          <code style='color:red;font-weight:normal;line-break:anywhere;padding:0px 0px 0px 0px'><?=$tokenJWT; ?></code>
                          <label id="tokenJWT" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-6">
                        <div class="form-group">
                          <label class="form-label">Token Start</label>
                          <span class="help"></span>
                          <div class="controls">
                            <input type="datetime-local" id="tokenStartDateTime" name="tokenStartDateTime"  class="form-control" style="font-weight:bold"  value="<?=$tokenStartDateTime; ?>">
                            <label id="tokenStartDateTime" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="col-md-6 col-sm-6 col-xs-6">
                        <div class="form-group">
                          <label class="form-label">Token End</label>
                          <span class="help"></span>
                          <div class="controls">
                            <input type="datetime-local" id="tokenEndDateTime" name="tokenEndDateTime"  class="form-control" style="font-weight:bold"  value="<?=$tokenEndDateTime; ?>">
                            <label id="tokenEndDateTime" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                          </div>
                        </div>
                      </div>
                  ...
                  <div class="row" style="margin-top:2%">
                    ...
                    <div class="col-md-2 col-sm-2 col-xs-2">
                      <div class="form-group">
                        ...
                        <span class="help"></span>
                        <div class="controls">
                          <select class="form-control" name="tokenStatus" id="tokenStatus">
                          <?php
                          if ($aTokensStatus) {
                          foreach ($aTokensStatus as $key => $value) {
                          $selected = ($key == $tokenStatus) ? 'selected' : '';
                    ...
                    <div class="col-md-8 col-sm-8 col-xs-8">
                      <div class="form-group">
                        <div class="controls">
                          ...
                          <?php echo (isset($dmID)) ? "<button class='btn btn-danger' name='btnDelete' style='width:100px;margin-right:20px'>Delete</button>" : null; ?>
    ...
    <script>
      $(document).ready(function() {
        ...
        $("input[name='tokenStartDateTime']").blur(function(e) {
          // insert tokenEndDateTime 2 years 
          let startDate = $(this).val();
          let startDateTime = new Date(startDate);
          let endDateTime = new Date(startDateTime);
          endDateTime.setFullYear(endDateTime.getFullYear() + 2);
          let endDate = endDateTime.toISOString().slice(0, 16);
          $("input[name='tokenEndDateTime']").val(endDate);
        ...
        function formValidate(){
          ...
          if ($("input[name=tokenName]").val() == "")  {
            err.push(['tokenName', 'Enter the Name / Reference of the Token']);
          }
