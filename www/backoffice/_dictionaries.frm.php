...
@define('PREVIUS_PAGE', '_dictionaries.lst');
@define('NEXT_PAGE',    '_dictionaries.sbm');
...
if ($id) {
    $action = 'upd';
    $model = (new SysDictionaries())->findById($id);
    foreach ($model->Data() as $field => $value) {
        ${$field} = ($field !== '') ? trim($value) : null;
    }
    $dictUUID = (empty($dictUUID)) ? $newUUID : $dictUUID;
}
            ...
            <div class="col-md-12">
              <div class="grid simple">
                ...
                <form name="form" action="<?php echo NEXT_PAGE; ?>" method="post" enctype="multipart/form-data">
                  ...
                  <input type="hidden" id="dictUUID" name="dictUUID" value="<?=$dictUUID; ?>">
                  <?php
                  if ($action = 'upd') {
                      echo ($dictID)   ? chr(10).chr(9).chr(9).chr(9).chr(9)."<input type='hidden' id='dictID' name='dictID' value='{$dictID}'>".chr(10) : null;
                  }// end if
                  ?>
                  <!-- row -->  
                  <div class="row">
                    ...
                    <div class="col-md-4 col-sm-4 col-xs-4">
                      <div class="form-group">                        
                        ...
                        <div class="controls">
                        <p style='font-family:roboto;color:#dadada;font-weight:bold'><?=$dictUUID; ?></p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="row">
                    <div class="col-md-8 col-sm-8 col-xs-8">
                        <div class="form-group">
                            <label class="form-label">Conteúdo em Inglês - EN</label>
                            <span class="help"></span>
                            <div class="controls">
                                <textarea id="dictContentEN" name="dictContentEN" class="form-control" rows="2" style="font-weight:bold;"><?= $dictContentEN ?></textarea>
                                <label id="dictContentEN" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- row -->  
                  <div class="row">
                    <div class="col-md-8 col-sm-8 col-xs-8">
                        <div class="form-group">
                            <label class="form-label">Conteúdo Traduzido para o Idioma</label>
                            <span class="help"></span>
                            <div class="controls">
                                <textarea id="dictContentTranslated" name="dictContentTranslated" class="form-control" rows="2" style="font-weight:bold;"><?= $dictContentTranslated ?></textarea>
                                <label id="dictContentTranslated" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- row -->  
                  <div class="row">
                    <div class="col-md-8 col-sm-8 col-xs-8">
                        <div class="form-group">
                            <label class="form-label">Informações / Observações dobre esta Tradução</label>
                            <span class="help">( opcional )</span>
                            <div class="controls">
                                <textarea id="dictInfo" name="dictInfo" class="form-control" rows="5" style="font-weight:normal;color:#696969"><?= $dictInfo ?></textarea>
                                <label id="dictInfo" style="padding-left:5px;font-size:12px" class="form-label error"></label>
                            </div>
                        </div>
                    </div>
                </div>
                  ...
                  <div class="row" style="margin-top:2%">
                    ...
                    <div class="col-md-2 col-sm-2 col-xs-2">
                      <div class="form-group">
                        ...
                        <span class="help"></span>
                        <div class="controls">
                          <select class="form-control" name="dictStatus" id="dictStatus">
                          <?php
                          if ($aDictionariesStatus) {
                          foreach ($aDictionariesStatus as $key => $value) {
                          $selected = ($key == $dictStatus) ? 'selected' : '';
                          echo "<option value='$key' $selected >$value</option>";
                          }
                          } // end iff
                          ?>
                    ...
                    <div class="col-md-8 col-sm-8 col-xs-8">
                      <div class="form-group">
                        <div class="controls">
                          ...
                          <?php echo (isset($dictID)) ? "<button class='btn btn-danger' name='btnDelete' style='width:100px;margin-right:20px'>Excluir</button>" : null; ?>
    ...
    <script>
      $(document).ready(function() {
        ...
        function formValidate(){
          ...
          if ($("textarea[name=dictContentEN]").val() == "")  {
            err.push(['dictContentEN', 'Infome o conteúdo em Inglês']);
          }
          if ($("select[name=langCode]").val() == "")  {
            err.push(['langCode', 'Infome idioma da Tradução']);
          }
          if ($("textarea[name=dictContentTranslated]").val() == "")  {
            err.push(['dictContentTranslated', 'Infome o conteúdo traduzido do Inglês para o idioma selecionado']);
          }
          // end validacoes
