...
@define('PAGE_NAME', $dict->translate('SHOP', $langCode));
@define('PREVIUS_PAGE', null);
@define('NEXT_PAGE', '_shops.frm');

@define('MODEL', 'Shops');
@define('statusField', 'shopStatus');
@define('fieldQuery', 'shopUUID');
                      ...
                      <thead>
                        ...
                        if ($records) {
                          foreach ($records as $record) {
                                ?> <tr>
                        <td style="text-align:center;vertical-align:middle"><?= $record->shopID ?></td>
                        <!-- <td style="text-align:center;vertical-align:middle"><?= $record->shopUUID ?></td> -->
                        <td style="vertical-align:middle;font-weight:bold"><?= $record->shopName ?></td>
                        <td style="text-align:center;vertical-align:middle"><?= f_filesSizeFormatShort($record->shopFileSize) ?></td>
                        <td style="text-align:center;vertical-align: middle;font-weight:normal"><?=$record->shopFileDimensions.' px'?></td>

                        <td style="text-align:center;vertical-align:middle">
                          <a href="#" name="changeThisStatus" title='<?=$dict->translate('Click here to Change Status!',$langCode)?>'data="<?=$record->shopUUID?>,<?=statusField?>" style='border-radius:10px;padding:8%' class="label <?= $aShopsStatusLabel[$record->shopStatus] ?>" >
                            <?= $aShopsStatus[$record->shopStatus] ?>
                          </a>
                        </td>
                        
                        <td style="text-align:center;vertical-align:middle">
                          <a href="<?= NEXT_PAGE ?>?ref=<?= f_encode($record->shopID) ?>"
