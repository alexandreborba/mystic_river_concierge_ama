                  ...
                  <table class="table table-striped" id="datalist">
                      <thead>
                          <tr>
                              ...
                              <th style="text-align:left;width:auto">NAME - Use / Need / Application or Reference</th>
                        ...
                        <?php 
                            ...
                            $tokens = (new SysTokens())->find("shipCode=:ship AND deleted_at IS NULL","ship={$sCode}")->fetch(true);
                            if($tokens){
                                foreach($tokens as $token){
                                ?>
                                    <tr class="odd gradeX">
                                        <td style="text-align:center;vertical-align: middle;"><?=$token->tokenID?></td>
                                        <!-- 
                                        <td style="text-align:center;vertical-align: middle;">
                                        <a href="<?=NEXT_PAGE?>?ref=<?=f_encode($token->tokenID)?>" ><?=$token->tokenUUID?></a>
                                        </td>
                                        -->
                                        <td style="text-align:center;vertical-align: middle;font-weight:normal"><?=$token->tokenDateTime?></td>
                                        <td style="text-align:left;vertical-align: middle;font-weight:normal"><?=$token->tokenName?></td>
                                        <!-- <td style="text-align:left;vertical-align: middle;font-weight:normal"><?=$token->tokenJWT?></td> -->
                                        
                                        <td style="text-align:center;vertical-align: middle;">
                                        <span style="padding:10px 30px 10px 30px;" class="label <?=$aTokensStatusLabel[$token->tokenStatus]?>"><?=$aTokensStatus[$token->tokenStatus]?></span>
                                        </td>
                                        <td class="center" style="text-align:center;vertical-align: middle;">
                                            <a href="<?=NEXT_PAGE?>?ref=<?=f_encode($token->tokenID)?>" class="btn btn-success" style="width:100px;">Change</a>
                                        </td>
                                    </tr>
