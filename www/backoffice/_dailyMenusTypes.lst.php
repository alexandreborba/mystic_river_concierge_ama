...
@define('PAGE_NAME', $dict->translate('DAILY MENUS TYPES', $langCode));
@define('PREVIUS_PAGE', null);
@define('NEXT_PAGE', '_dailyMenusTypes.frm');

@define('MODEL', 'DailyMenusTypes');
@define('statusField', 'dmTypeStatus');
@define('fieldQuery', 'dmTypeUUID');
$sCode = shipCode;
?>
                  ...
                  <table class="table table-striped" id="datalist">
                      <thead>
                        ...
                        <?php 
                            ...
                            $dailyMenuTypesTypes = (new DailyMenusTypes())->find("shipCode=:ship AND deleted_at IS NULL","ship={$sCode}")->fetch(true);
                            if($dailyMenuTypesTypes){
                              
                        foreach($dailyMenuTypesTypes as $dailyMenuType){
                    ?>
                    <tr class="odd gradeX">
                        <td style="text-align:center;vertical-align: middle;"><?=$dailyMenuType->dmTypeID?></td>
                        <td style="text-align:center;vertical-align: middle;"><?=$dailyMenuType->dmTypeUUID?></td>
                        
                        <td style="text-align:left;vertical-align: middle;font-weight:bold"><?=$dailyMenuType->dmTypeName?></td>
                        
                        <td style="text-align:center;vertical-align:middle">
                          <a href="#" name="changeThisStatus" title='<?=$dict->translate('Click here to Change Status!',$langCode)?>'data="<?=$dailyMenuType->dmTypeUUID?>,<?=statusField?>"  style='border-radius:10px;padding:8%' class="label <?= $aDmStatusLabel[$dailyMenuType->dmTypeStatus] ?>" >
                            <?= $aDmStatus[$dailyMenuType->dmTypeStatus] ?>
                          </a>
                        </td>
                        
                        <td class="center" style="text-align:center;vertical-align: middle;">
                            <a href="<?=NEXT_PAGE?>?ref=<?=f_encode($dailyMenuType->dmTypeID)?>" class="btn btn-success" style="width:100px;"><?=$dict->translate('Change',$langCode);?></a>
                        </td>
                      </tr>
