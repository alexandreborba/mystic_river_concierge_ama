...
@define('PAGE_NAME', $dict->translate('MANAGERS', $langCode));
@define('PREVIUS_PAGE', null);
@define('NEXT_PAGE', '_managers.frm');

@define('MODEL', 'SysManagers');
@define('statusField', 'managerStatus');
@define('fieldQuery', 'managerUUID');
    ...
    <div class="page-container row-fluid">
      ...
      <div class="page-content">
        ...
        <div class="content">
          ...
          <?php if($_SESSION["messages_form"]["text"]) { ?>
            <div class="alert alert-<?=$_SESSION["messages_form"]["tp"]?> ">
              <button class="close" data-dismiss="alert"></button>
              <?=$_SESSION["messages_form"]["text"]?>
              <?php unset($_SESSION["messages_form"]); ?>
            </div>
          <?php } // end if ?>
                  ...
                  <table class="table table-striped" id="datalist">
                    <thead>
                      <tr>
                        ...
                        <th style="text-align:left;width:35%"><?=$dict->translate('name', $langCode)?></th>
                        <th style="text-align:left;width:35%"><?=$dict->translate('email', $langCode)?></th>
                    ...
                    ?> 
                    <tr class="odd gradeX">
                        <td style="text-align:center;vertical-align: middle;"><?=$record->managerID?></td>
                        <td style="text-align:center;vertical-align: middle;">
                          <a href="_managers.frm?ref=<?=f_encode($record->managerID)?>" ><?=$record->managerUUID?></a>
                        </td>
                        <td style="text-align:left;vertical-align: middle;font-weight:bold"><?=$record->managerName?></td>
                        <td style="text-align:left;vertical-align: middle;font-weight:normal"><?=$record->managerEmail?></td>

                        <td style="text-align:center;vertical-align:middle">
                            <a href="#" name="changeThisStatus" title='<?=$dict->translate('Click here to Change Status!',$langCode)?>'data="<?=$record->managerUUID?>,<?=statusField?>" style='border-radius:10px;padding:8%' class="label <?= $aManagersStatusLabel[$record->managerStatus] ?>" >
                              <?= $dict->translate($aManagersStatus[$record->managerStatus],$langCode); ?>
                            </a>
                          </td>

                        <td class="center" style="text-align:center;vertical-align: middle;">
                            <a href="<?=NEXT_PAGE?>?ref=<?=f_encode($record->managerID)?>" class="btn btn-success" style="width:100px;">Editar</a>
                        </td>
                      </tr>
